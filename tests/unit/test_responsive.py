...
class ResponsiveLayoutTests(unittest.TestCase):
    ...
    def test_keeps_current_profile_while_width_jitters_near_breakpoint(self) -> None:
        self.assertEqual(
            LayoutProfile.stable_from_width(970, current_mode="regular").mode,
            "regular",
        )
        self.assertEqual(
            LayoutProfile.stable_from_width(947, current_mode="regular").mode,
            "compact",
        )
        self.assertEqual(
            LayoutProfile.stable_from_width(1000, current_mode="compact").mode,
            "compact",
        )
        self.assertEqual(
            LayoutProfile.stable_from_width(1012, current_mode="compact").mode,
            "regular",
        )
        self.assertEqual(
